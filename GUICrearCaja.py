...
class CrearCajaApp:
    ...
    def abrir_ventana_crear_pos(self):
        ...
        self.width = ventana_pos.winfo_reqwidth()
        self.height = ventana_pos.winfo_reqheight()

        if ventana_pos is not None:
            screen_width = ventana_pos.winfo_screenwidth()
            screen_height = ventana_pos.winfo_screenheight()

            spawn_x = int((screen_width - self.width) / 2)
            spawn_y = int((screen_height - self.height) / 2)

            ventana_pos.geometry("+{}+{}".format(spawn_x, spawn_y))
        else:
    ...
    def centrar_ventana(self):
        self.width = self.ventana_creacion_caja.winfo_reqwidth()
        self.height = self.ventana_creacion_caja.winfo_reqheight()

        if self.ventana_creacion_caja is not None:
