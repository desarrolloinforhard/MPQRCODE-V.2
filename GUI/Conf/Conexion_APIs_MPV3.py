...
#NUEVAS REQUEST QUE UTLIZAN LA NUEVA VERSION DE API DE MP TANTO QR COMO POINT
    def QR_Crear_order(self, datos_orden):
        try:
            ...
            payload = {
                'type': 'qr',
                'total_amount': f'{datos_orden['monto_pagar']}',
                'external_reference': datos_orden['nro_factura'],
                'config': {
                    'qr': {
                        'external_pos_id': datos_orden['external_id_pos'],
                        'mode': 'hybrid'
                    }
                },
                "transactions": {
                    "payments": [
                    {
                        "amount": f'{datos_orden['monto_pagar']}'
                    }
                    ]
                }
            }
            ...
            response = requests.post(url, headers=headers, data=json.dumps(payload))
            print(response.json())
            guardar_json(response.json(), f"QR_Crear_order_{datos_orden['nro_factura']}.json")
            return response
        except Exception as e:
            print(e)
            
            
    def QR_Obtener_order_por_ID(self, order_id):
        try:
            ...
            if response.status_code < 300:
                guardar_json(response.json(), f"QR_Obtener_order_por_ID_{response.json()['external_reference']}.json")
            return response
        except Exception as e:
            print(e)
            
    def QR_Cancelar_order_por_ID(self, order_id):
        try:
            ...
            print(response)
            if response.status_code < 300:
                guardar_json(response.json(), f"QR_Cancelar_order_por_ID_{response.json()['external_reference']}.json")
            return response
        except Exception as e:
            print(e)
            
    def QR_Reembolsar_una_order(self, order_id):
        try:
            url = f'https://api.mercadopago.com/v1/orders/{order_id}/cancel'
            ...
            print(response.json())
            if response.status_code < 300:
                guardar_json(response.json(), f"QR_Reembolsar_una_order_{response.json()['external_reference']}.json")
            return response
        except Exception as e:
            print(e)
