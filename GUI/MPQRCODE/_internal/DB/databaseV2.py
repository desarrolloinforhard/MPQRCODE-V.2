...
class ConexionSybase:
    ...
    def ejecutar_consulta(self, sentencia_sql: str, params=None, as_dict: bool = True, scalar: bool = False):
        print("[SQL]", sentencia_sql)
        print("[PARAMS]", [(type(p).__name__, repr(p)) for p in (params or ())])
        ...
        try:
            ...
            with self.conexion.cursor() as cursor:
                ...
                if params is not None:
                    if isinstance(params, list):
                        ...
                        params = (params,)
                    # convertir bool→int y datetimes con tz→naive
                    norm = []
                    for p in params:
                        if isinstance(p, bool):
                            norm.append(int(p))
                        elif hasattr(p, 'tzinfo') and p.tzinfo is not None:
                            norm.append(p.replace(tzinfo=None))
                        else:
                            norm.append(p)
                    params = tuple(norm)

                is_select = self._is_select(sentencia_sql)
                cursor.execute(sentencia_sql, params) if params is not None else cursor.execute(sentencia_sql)

                if is_select:
                    rows = cursor.fetchall()
                    if not rows:
                        return [] if not scalar else None
                    if scalar:
                        return rows[0][0]
                    if as_dict:
            ...
            error_traceback = traceback.format_exc()
            print(f"[ejecutar_consulta] ODBC Error: {e}\nTraceback:\n{error_traceback}")
            return None
        except Exception as e:
            error_traceback = traceback.format_exc()
            print(f"[ejecutar_consulta] Error inesperado: {e}\nTraceback:\n{error_traceback}")
            return None
    ...
    def _to_sql_literal(self, v):
        """Convierte valor Python a literal SQL (sin parametrizar)."""
        if v is None:
            return "NULL"
        ...
        if isinstance(v, (int, float)):
            return str(v)
        # Escapar comillas simples en texto
        return f"'{str(v).replace("'", "''")}'"

    def insertar_datos_POINT(self, tabla: str, datos: dict) -> bool:
        if not datos:
            print("insertar_datos_POINT: dict vacío.")
            return False

        # Clave: primero por id_pago, si no hay, por external_reference
        where = None
        if 'id_pago' in datos and datos['id_pago']:
            where = f"id_pago = {self._to_sql_literal(datos['id_pago'])}"
        elif 'external_reference' in datos and datos['external_reference']:
            where = f"external_reference = {self._to_sql_literal(datos['external_reference'])}"
        ...
        try:
            ...
            with self.conexion.cursor() as cursor:
                ...
                if existe:
                    set_clause = ", ".join(f"{k} = {self._to_sql_literal(v)}" for k, v in datos.items())
                    sql = f"UPDATE {tabla} SET {set_clause} WHERE {where}"
                else:
                    columnas = ", ".join(datos.keys())
                    valores  = ", ".join(self._to_sql_literal(v) for v in datos.values())
            ...
            print(f"[insertar_datos_POINT] Error: {e}\nSQL: {sql}")
            return False


    """def insertar_datos_POINT(self, tabla, datos):
        try:
            self.conectar()
            with self.conexion.cursor() as cursor:
                # Convertir datos en un JSON string
