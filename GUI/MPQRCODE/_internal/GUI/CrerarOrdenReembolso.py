...
class CrearOrdenReembolso(BarraProgreso):
    ...
    def actaulizar_nuevos_datos(self):
        try:
            ...
            self.datos_pago = self.DICT_CONEXION["conexionAPI"].obtenerPago_manual(self.DICT_DATOS_ORDEN)
    ...
    def finalizar_pago(self):
        try:
            if self.datos_pago["payment_method_id"] == "account_money" or self.datos_pago["payment_method_id"] == "consumer_credits":
                pass
            elif self.datos_pago["payment_method_id"] == "interop_transfer":
                self.payment_method_id = self.datos_pago["payment_method_id"]
                self.payment_method_name = "MP Billetera Virtual" #03/07/25
                self.payment_type_id = self.datos_pago["payment_type_id"]
            else:
                ...
                for pagos in pagos_disponibles.json():
                    if pagos["id"] == self.datos_pago["payment_method_id"]:
                        name_tarjeta_encontrada = pagos['name']
                self.card_first_six_digits = self.datos_pago["card"]["first_six_digits"]
                self.card_last_four_digits = self.datos_pago["card"]["last_four_digits"]
                self.payment_method_id = self.datos_pago["payment_method_id"]
