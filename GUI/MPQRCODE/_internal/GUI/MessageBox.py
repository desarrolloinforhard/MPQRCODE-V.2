...
class CustomMessageBox:
    ...
    def close(self):
        ...
        if self.msg_type in ("yesno", "continuar_cancelar") and self.result is None:
            # si cerraron la ventana sin elegir, trátalo como Cancelar
            self.result = False
        ...
        try:
            ...
            # restaurar teclas personalizadas
            try:
                self._restaurar_teclas()
            except Exception:
                pass
    ...
    def create_buttons(self, content_frame, msg_type, font_size):
        ...
        if msg_type in ["info", "warning", "error"]:
            ...
            ttk.Button(frame, text="Sí", command=lambda: self.set_result(True),
                    style="Custom.TButton", bootstyle="primary").pack(side="left", padx=10)
            ttk.Button(frame, text="No", command=lambda: self.set_result(False),
                    style="Custom.TButton", bootstyle="danger").pack(side="left", padx=10)

        elif msg_type == "continuar_cancelar":
            # Orden solicitado: Cancelar (izq) / Continuar (der)
            if self.teclado is None:
                ttk.Button(frame, text="Cancelar", command=lambda: self.set_result(False),
                        style="Custom.TButton", bootstyle="secondary").pack(side="left", padx=10)
                ttk.Button(frame, text="Continuar", command=lambda: self.set_result(True),
                        style="Custom.TButton", bootstyle="success").pack(side="right", padx=10)
            else:
                ttk.Button(frame, text="Cancelar (Clear)", command=lambda: self.set_result(False),
                    style="Custom.TButton", bootstyle="secondary").pack(side="left", padx=10)
                ttk.Button(frame, text="Continuar (Sub Total)", command=lambda: self.set_result(True),
                        style="Custom.TButton", bootstyle="success").pack(side="right", padx=10)
                
        elif msg_type == "reintentar_cancelar":
            # Orden: Cancelar (izq) / Reintentar (der)
            if self.teclado is None:
                ttk.Button(frame, text="Cancelar", command=lambda: self.set_result(False),
                        style="Custom.TButton", bootstyle="secondary").pack(side="left", padx=10)
                ttk.Button(frame, text="Reintentar", command=lambda: self.set_result(True),
                        style="Custom.TButton", bootstyle="success").pack(side="right", padx=10)
            else:
                # 6a = Reintentar (Sub Total), 86 = Cancelar (Clear)
                ttk.Button(frame, text="Cancelar (Clear)", command=lambda: self.set_result(False),
                        style="Custom.TButton", bootstyle="secondary").pack(side="left", padx=10)
                ttk.Button(frame, text="Reintentar (Sub Total)", command=lambda: self.set_result(True),
                        style="Custom.TButton", bootstyle="success").pack(side="right", padx=10)
