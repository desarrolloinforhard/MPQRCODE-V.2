...
class TopLevelCargaMenu():
    def __init__(self, master, conexionAPI, tipo_carga, **kwargs):
        ...
        self.aviso_actual = 0
        self.avisos = []
        if tipo_carga == 'crear_sucursal': 
            self.avisos = ["Enviando datos", "Creando Sucursal", "Esperando Respuesta", "Registrando en el DBA"]
        elif tipo_carga == 'crear_orden':
    ...
    def iniciar_interfaz(self):
        self.label_aviso()  # Aquí se llama a la función que crea un widget de CTkLabel
    ...
    def envio_a_MercadoPago(self, datos_sucursal):
        ...
        if not self.master == None:            
            while resultado_envio and self.hilo_activo:  # Utiliza la bandera para controlar la ejecución del bucle
                if self.numero_deseado == 1:
                    self.aviso_creado.wait()  # Espera a que se haya creado el aviso antes de actualizarlo
            ...
            while resultado_envio and self.hilo_activo:  # Utiliza la bandera para controlar la ejecución del bucle
                if self.numero_deseado == 1:
                    self.actualizar_numero_deseado(1)
                resultado_envio = False
                time.sleep(5)
                break
            self.hilo_activo = False  # Establece la bandera en False cuando el hilo termina su ejecución
            
# Crear instancia de la clase TopLevelCargaMenu
#TopLevelCargaMenu(None, None, 'crear_orden')
