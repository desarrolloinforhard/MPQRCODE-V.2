...
#------------------------------------------------------------ CIERRES --------------------------------------------------------------
    ...
    def avisar_pago_encontrado(self, origen: str, pago_id: str):
        """
        Muestra un aviso visual cuando se detectó un pago.
        Por ahora actualiza la barra/progreso y deja log.
        """
        try:
            self.DICT_PROGRESO["text_label_aviso"] = f"Pago encontrado ({origen})"
            self.DICT_PROGRESO["carga"] = 50
            self.DICT_PROGRESO["command"] = None
            self.progreso(**self.DICT_PROGRESO)

            # Si querés mostrar el id en pantalla
            if "id_pago" in self.DICT_WIDGETS:
                try:
                    self.DICT_WIDGETS["id_pago"].config(text=f"Nro. OP: {pago_id}")
                except Exception:
                    pass

            log_error(f"Pago encontrado desde {origen}. ID={pago_id}", "avisar_pago_encontrado")
            print(f"✅ Pago encontrado en {origen}: ID {pago_id}")
