...
from GUI.MessageBox import CustomMessageBox, mostrar_error_y_cerrar
...
class CrearOrdenPago(BarraProgreso):
    def __init__(self, frame, DICT_WIDGETS, DICT_DATOS_ORDEN, DICT_CONEXION):
        try:
            ...
            self.condicion_teclado = self.condicion_teclado if self.condicion_teclado else False
    ...
    def crear_orden(self):
        try:
            ...
            if self.qr_data is not None and self.qr_data.status_code == 200:
                ...
                """self.DICT_WIDGETS["root"].after(
                    100,
                    lambda: CustomMessageBox(
                        self.DICT_WIDGETS["root"],
                        "ERROR",
                        f"Fallo al crear orden después de {reintentos} intentos.\n{error_envio}",
                        "error",
    ...
    def validate_number(self, entry_value):
        try:
            ...
            if entry_value == "" or entry_value.isdigit() or entry_value == self.placeholder_text:
                ...
                self.DICT_WIDGETS["root"].after(100, lambda: CustomMessageBox(self.DICT_WIDGETS["root"], "Error", "Solo puedes ingresar valores numéricos.", "error", teclado_hasar=self.teclado_hasar))
                self.ventana_buscarpagomanual_abierta = False
    ...
    """def buscar_id_pago_manual(self):
        try:
            ...
            if respuesta == True:
                self.DICT_WIDGETS["root"].after(100, lambda: CustomMessageBox(self.DICT_WIDGETS["root"], "Nro Operacion exitosa", "Se ha encontrado el número de operación", "info", teclado_hasar=self.teclado_hasar))
                ...
                self.DICT_WIDGETS["root"].after(100, lambda: CustomMessageBox(self.DICT_WIDGETS["root"], "Error", "No se ha encontrado el número de operación", "error", teclado_hasar=self.teclado_hasar))
                #messagebox.showerror("Error", "No se ha encontrado el número de operación")
            else:
                self.DICT_WIDGETS["root"].after(100, lambda: CustomMessageBox(self.DICT_WIDGETS["root"], "Error", respuesta, "error", teclado_hasar=self.teclado_hasar))
        ...
        try:
                    ...
                    if self.DICT_DATOS_ORDEN["nro_factura"] != self.datos_pagos["external_reference"]:
                        if self.es_fecha_aprovado(self.datos_pagos['date_approved']) <= 300:
                            ...
                            if respuesta.status_code == 200:
                                ...
                                print(4)
                                self.DICT_WIDGETS["root"].after(100, lambda: CustomMessageBox(self.DICT_WIDGETS["root"], f"Error API", respuesta.json(), "error", teclado_hasar=self.teclado_hasar))
                            ...
                            self.NRO_ERROR = 13
                            print(5)
                            self.DICT_WIDGETS["root"].after(100, lambda: CustomMessageBox(self.DICT_WIDGETS["root"], f"Error {self.NRO_ERROR}", self.LISTADO_ERRORES(), "error", teclado_hasar=self.teclado_hasar))
                            self.DICT_PROGRESO["text_label_aviso"] = "ERROR"
                            self.esperando_comparacion = True
                            self.reintentarpago = False
                            return
            ...
            if int(self.DICT_DATOS_ORDEN["response"]) == 98:
                ...
                if self.comprobación_PBS():  # <-- Si coincide, OMITIR devolución
                    ...
                    respuesta_reembolso = respuesta.status_code == 200
...
#------------------------------------------------------------ CIERRES --------------------------------------------------------------
    def cierre_ERROR(self, paso_M, label_error=None):
        ...
        self.DICT_PROGRESO["text_label_aviso"] = label_error if label_error else "ERROR"
        self.DICT_PROGRESO["carga"] = 99
        self.progreso(**self.DICT_PROGRESO)
        ...
        if paso_M:
            time.sleep(1)  # (opcional, solo si querés un retardo)
    ...
    def mostrar_error(self):
        print(11)
        # Mostrar sin `after`, y con espera para que sea bloqueante
        ...
        msg.show()  # ⬅️ Bloquea hasta que el usuario lo cierre manualmente

        # Luego de cerrarse el mensaje, actualiza la base
        self.DICT_CONEXION["conexionDBA"].actualizar_datos_condicion(
            "MPQRCODE_CONEXIONPROGRAMAS",
            self.datos_error,
            "nro_factura",
            f"'{self.DICT_DATOS_ORDEN['nro_factura']}'"
        )
    ...
    def cancelar_orden(self):
        try:
            ...
            print(error_str)
            log_error(error_str, "cancelar_orden")
