...
class TecladoHasar:
    def __init__(self, asignaciones_personalizadas=None, teclado_hasar_activo=False):
        ...
        self.funciones_teclado_global = {}        # keysym -> funcion
        self.funciones_teclado_por_entry = {}     # entry_widget -> {keysym -> funcion}
        ...
        self.map_letras = asignaciones_personalizadas or {}   # letras (KM_84_LETTER)
        self.map_funciones = {}                               # funciones (KM_84)
        ...
        teclas_letras = {
            ...
            "42": (66,),
            "43": (67,),
            "44": (68, 84),     # 👈 MISMA TECLA (dos bancos)
            "45": (69,),
            "46": (70,),
            "47": (71,),
            "48": (72,),
            "49": (73,),
            "4a": (74, 75),     # 👈 MISMA TECLA
        }

        self._ctx_stack = []   # cada item: {"id": int, "name": str, "hasar": {combo: fn}, "normal": {keysym: fn}}
        self._ctx_id_seq = 0        
    ...
    def asignar_entry(self, entry_widget):
        ...
        def on_key(event):
            # ----------------------------
            # MODO TECLADO NORMAL (HASAR OFF)
            # ----------------------------
            if not self.teclado_hasar_activo:
                ks = event.keysym  # 'Return', 'KP_Enter', 'Escape', etc.
                
                # 0) CONTEXTO ACTIVO (Return/Escape) - prioridad
                ctx = self._top_context()
                if ctx:
                    fn = ctx.get("normal", {}).get(ks)
                    if fn:
                        fn()
                        return "break"


                # 1) primero, funciones por entry
                if entry_widget in self.funciones_teclado_por_entry:
                    fmap = self.funciones_teclado_por_entry[entry_widget]
                    if ks in fmap:
                        fmap[ks]()     # ejecuta
                        return "break" # corta el evento (evita beep/acciones default)

                # 2) después, funciones globales
                if ks in self.funciones_teclado_global:
                    self.funciones_teclado_global[ks]()
                    return "break"

                return  # no hacemos nada: deja escribir normal
    ...
    def set_modo_teclado_hasar(self, enabled: bool):
        """
        Activa/desactiva el procesamiento Hasar.
        - enabled=True: interpreta combinaciones y consume teclas.
        - enabled=False: teclado normal (Return/Escape, etc).
        """
        self.teclado_hasar_activo = bool(enabled)
        # si activamos Hasar, forzamos modo Hasar (no escáner) para evitar estados raros
        if self.teclado_hasar_activo:
            self.activar_modo_hasar()
        print(f"[HASAR] teclado_hasar_activo={self.teclado_hasar_activo}")
    ...
    def procesar_tecla(self, tecla, estado):
        ...
        if len(estado["buffer"]) == 2:
            ...
            # 0) CONTEXTO ACTIVO (prioridad máxima)
            ctx = self._top_context()
            if ctx and combinacion in ctx.get("hasar", {}):
                fn = ctx["hasar"][combinacion]
                try:
                    fn()
                except Exception as e:
                    print("[HASAR] Error en handler de contexto:", e)
                estado["buffer"].clear()
                return None
            ...
            if numero:
                estado["buffer"].clear()
                if numero in self.map_letras:
                    return self.map_letras[numero]
                return None
    ...
    def push_context(self, name="CTX", hasar_map=None, normal_map=None):
        """
        hasar_map: dict {"86": fn_enter, "6a": fn_esc, "4a": fn_special}
        normal_map: dict {"Return": fn_enter, "Escape": fn_esc}
        Devuelve token/id para poder hacer pop seguro.
        """
        self._ctx_id_seq += 1
        token = self._ctx_id_seq
        self._ctx_stack.append({
            "id": token,
            "name": name,
            "hasar": dict(hasar_map or {}),
            "normal": dict(normal_map or {}),
        })
        print(f"[HASAR] push_context {name} id={token} depth={len(self._ctx_stack)}")
        return token

    def pop_context(self, token):
        for i in range(len(self._ctx_stack)-1, -1, -1):
            if self._ctx_stack[i]["id"] == token:
                ctx = self._ctx_stack.pop(i)
                print(f"[HASAR] pop_context {ctx['name']} id={token} depth={len(self._ctx_stack)}")
                return True
        return False
    ...
    def asignar_funcion_personalizada(self, combinacion, funcion, tecla_teclado=None, entry_widget=None):
        """
        combinacion: '4a', 'ab', etc (modo Hasar)
        tecla_teclado: keysym o lista/tupla de keysym (modo teclado normal)
                    ej: 'Return', 'KP_Enter', 'Escape'
        entry_widget: si se pasa, la tecla normal aplica SOLO a ese Entry.
                    si es None, aplica global (bind_all).
        """
        if not callable(funcion):
            return None

        # 1) Registrar modo Hasar (si está activo, se usará; si no, queda guardado igual)
        self.funciones_personalizadas[combinacion] = funcion

        # 2) Registrar tecla(s) de teclado normal (opcional)
        if tecla_teclado:
            if isinstance(tecla_teclado, (list, tuple, set)):
                teclas = list(tecla_teclado)
            else:
                teclas = [tecla_teclado]

            if entry_widget is not None:
                if entry_widget not in self.funciones_teclado_por_entry:
                    self.funciones_teclado_por_entry[entry_widget] = {}
                for ks in teclas:
                    self.funciones_teclado_por_entry[entry_widget][str(ks)] = funcion
            else:
                for ks in teclas:
                    self.funciones_teclado_global[str(ks)] = funcion
    ...
    def attach_widget(self, widget):
        """
        Adjunta la escucha de teclado Hasar a CUALQUIER widget (Toplevel/Frame/etc).
        Útil para modales: mientras el foco esté en el modal, igual capturamos combos.
        Devuelve una función detach() para desinstalar.
        """
        if widget is None:
            return lambda: None
        ...
        def on_key(event):
            # TECLADO NORMAL (Hasar OFF): solo contexto
            if not self.teclado_hasar_activo:
                ks = event.keysym
                ctx = self._top_context()
                if ctx:
            ...
            # HASAR ON
            ch = event.char.lower() if event.char else None
            if not ch:
                return "break"  # importante: consumimos

            # usamos un "estado fake" (no escribimos en entry)
            estado = {"buffer": buffer_local, "contenido_actual": ""}
            res = self.procesar_tecla(ch, estado)

            # si disparó contexto/acción, consumir
            return "break"
