...
const rutaMPPOINT = express.Router();
...
async function obtenerDatosUsuario() {
  try {
    ...
    const tokenPOINT = await conexion_sybase.obtenerTokenPoint();
    console.log(idUsuario, tokenPOINT)
    return { idUsuario, tokenPOINT };
  } catch (error) {
...
obtenerDatosUsuario()
    .then(datos => {
        ...
        respuestaAPIsMP = new Conexion_Api(datos.idUsuario, datos.tokenPOINT);
...
// Ruta MPPOINT
rutaMPPOINT.get('/', (req, res) => {
    res.end('Routing /api/MPQRPOINT para metodos POST: HABILITADO ');
});

rutaMPPOINT.post('/', async (req, res) => {
  try {
      const data = req.body;      
      if ('action' in data) {
        console.log(data);
              ...
              try {
                  ...
                  if (resultado.status == 'refunded' && resultado.status_detail == 'refunded') {
                    ...
                    *************************************
                    ...
                    --------------------------------------\n`);
                    conexion_sybase.posObtenerPagoPOINT(resultado, data.data.id, data);
                } else {
                    console.log(`\n
                    *************************************
              ...
              setTimeout(async () => {
                try {
                    ...
                    if (resultado.status == 'approved' && resultado.status_detail == 'accredited') {
                      console.log(`\n
                      *************************************
                      Aviso de pago con Point: RECIBIDO
                      *************************************
                      Factura: ${resultado.external_reference}
                      Monto: $${resultado.transaction_amount}
                      Sucursal: ${resultado.description}
                      ...
                      Fecha de Recepción: ${fechaActual}
                      Hora de Recepción: ${horaActual}
                      --------------------------------------\n`);// agregar luego = Caja: ${infoPOS.name}
                      conexion_sybase.posObtenerPagoPOINT(resultado, data.data.id, data);
                  } else if ('message' in resultado && resultado.message === 'Payment not found'){
                    console.error('\nEl pago no ha sido encontrado en la API de Mercado Pago');
                  } else {
                      console.log(`\n
                      *************************************
                      Aviso de pago con Código QR: ERROR
      ...
      }/*else if ('payment' in data && data.payment.id !== 0) {
        setTimeout(async () => {
          const resultado = await respuestaAPIsMP.obtener_pago(data.payment.id);
