...
class BuscarOrdenPago(BarraProgreso):
    ...
    def mostrar_error(self):
        try:
            mostrar_error_y_cerrar(self.DICT_WIDGETS, f"Error {self.NRO_ERROR}\n{self.LISTADO_ERRORES()}", teclado_hasar=self.teclado_hasar)
            """CustomMessageBox(self.DICT_WIDGETS["root"], self.LISTADO_ERRORES(), f"Error {self.NRO_ERROR}", "error", teclado_hasar=self.teclado_hasar)"""
...
class Ventana_BuscarPagoManual:
    def __init__(self, DICT_WIDGETS, DICT_DATOS_ORDEN, DICT_CONEXION, tipo_busqueda=False): #AGREGAR EL DICT__DATOS, PARA REALIZAR LA COMPARACION DEL NRO FACTURA Y EL MONTO...
        ...
        if not self.condicion_teclado or self.condicion_teclado == None:
            ...
            self.teclado_hasar.asignar_funcion_personalizada("4a", self.press_enter_buscar_id_manual)
            self.teclado_hasar.registrar_combinacion_personalizada("ab")
            self.teclado_hasar.asignar_funcion_personalizada("ab", self.on_closing)

            self.teclado_hasar.asignar_entry(self.DICT_WIDGETS["def_ventana_buscar_pago_manual"]["entry_info_buscar_pago"])
            pass
        ...
        try:
            ...
            if datos_pagos.status_code >= 200 and datos_pagos.status_code < 300:
                if datos_pagos.json()["point_of_interaction"]["type"] == "POINT" :
                    ...
                    self.SELECCION_TIPO_BUSQUEDA = True
                self.DICT_DATOS_ORDEN["respuesta"] = datos_pagos          
                if self.SELECCION_TIPO_BUSQUEDA:
                    print("QR")
                    self.DICT_DATOS_ORDEN["idINTEGRACION"] = 0
                    respuesta = self.DICT_CONEXION["conexionAPI"].obtenerPago_manual(self.DICT_DATOS_ORDEN)
                    #respuesta = self.DICT_CONEXION["conexionAPI"].obtenerPago_manual(datos_pagos)
                    ...
                    #respuesta = self.DICT_CONEXION["conexionAPIPOINT"].obtenerPago_manualPOINT(datos_pagos, self.DICT_DATOS_ORDEN)
            else:
                respuesta = "No se ha encontrado el número de operación"

            if isinstance(respuesta, str):
                mostrar_error_y_cerrar(self.DICT_WIDGETS, f"Error: {respuesta}", teclado_hasar=self.teclado_hasar)
                #CustomMessageBox(self.DICT_WIDGETS["root"], "Error", respuesta, "error", teclado_hasar=self.teclado_hasar)
                #messagebox.showerror("Error", respuesta)
                if not respuesta == "No se ha encontrado el número de operación":
                    self.respuesta_mp = False
            else:
                mostrar_error_y_cerrar(self.DICT_WIDGETS, "Se ha encontrado el número de operación", teclado_hasar=self.teclado_hasar, type_msg="info")
                #CustomMessageBox(self.DICT_WIDGETS["root"], "Operación Exitosa", "Se ha encontrado el número de operación", "info", teclado_hasar=self.teclado_hasar)
    ...
    def validate_number(self, entry_value):
        try:
            ...
            if entry_value == "" or entry_value.isdigit() or entry_value == self.placeholder_text:
                ...
                mostrar_error_y_cerrar(self.DICT_WIDGETS, f"Error\nSolo puedes ingresar valores numéricos.", teclado_hasar=self.teclado_hasar)
                #CustomMessageBox(self.DICT_WIDGETS["root"], "Error", "Solo puedes ingresar valores numéricos." "error",teclado_hasar=self.teclado_hasar)
                self.respuesta_mp = "CERRANDO"
