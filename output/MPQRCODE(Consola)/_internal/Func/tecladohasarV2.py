...
class TecladoHasar:
            ...
            if event.event_type == keyboard.KEY_DOWN:
                ...
                if len(self.current_keys) == 2:
                    ...
                    if combinacion in self.key_mapping:
                        ...
                        # Si no es combinación válida, insertar cada tecla individual
                        for k in self.current_keys:
                            if len(k) == 1:
                                self.insertar_en_entry(k)
                        print(f"[DEBUG] Combinación desconocida: {self.current_keys}")
    ...
    def escanear_codigo(self, timeout=5) -> str:
        """
        Escucha caracteres como un teclado normal hasta que se presione Enter.
        Devuelve el string escaneado o vacío si no se presiona Enter en el tiempo indicado.
        """
        print("[TECLADO] Modo escaneo directo (input libre) activado")
        self.activar_modo_normal()
        self.escuchando = False  # Pausa otras escuchas
        self.escaneo_activo = True
        ...
        codigo = "".join(buffer).upper()

        print(f"[TECLADO] Código escaneado: {codigo}")
        self.escaneo_activo = False
        self.escuchando = True
        self.desactivar_modo_normal()
        return codigo
